 ...
 * a mask based on the connection type*/
	...
	lcd_port_value=( lcd_port_value & 0x0F )|( a_command & 0xF0 );
#else
	lcd_port_value=( lcd_port_value & 0xF0 )|( ( a_command & 0xF0 )>>4);
#endif
	...
	lcd_port_value=GPIO_readPort(LCD_DATA_PORT_ID);
#ifdef LCD_LAST_PORT_PINS
	lcd_port_value=( lcd_port_value & 0x0F )|(( a_command & 0x0F )<<4);
#else
	lcd_port_value=( lcd_port_value & 0xF0 )|( a_command & 0x0F );
#endif
	/*send the command to port that is connected to the data bus*/
	GPIO_writePort(LCD_DATA_PORT_ID,lcd_port_value);
	/*delay 1 microsecond because tdse=100 nanoseconds*/
...
void LCD_displayCharacter(uint8 a_character){
	 ...
	 * a mask based on the connection type*/
		...
		lcd_port_value=GPIO_readPort(LCD_DATA_PORT_ID);
	#ifdef LCD_LAST_PORT_PINS
		lcd_port_value=( lcd_port_value & 0x0F )|( a_character & 0xF0 );
	#else
		lcd_port_value=( lcd_port_value & 0xF0 )|( ( a_character & 0xF0 )>>4);
	#endif
		/*send the command to port that is connected to the data bus*/
		GPIO_writePort(LCD_DATA_PORT_ID,lcd_port_value);
		...
		lcd_port_value=GPIO_readPort(LCD_DATA_PORT_ID);
	#ifdef LCD_LAST_PORT_PINS
		lcd_port_value=( lcd_port_value & 0x0F )|(( a_character & 0x0F )<<4);
	#else
		lcd_port_value=( lcd_port_value & 0xF0 )|( a_character & 0x0F );
	#endif
		/*send the command to port that is connected to the data bus*/
		GPIO_writePort(LCD_DATA_PORT_ID,lcd_port_value);
		/*delay 1 microsecond because tdse=100 nanoseconds*/
...
void LCD_moveCursor(uint8 a_row,uint8 a_col){
	...
	case 0:
		lcd_memory_address=a_col+FIRST_ROW_ADDRESS;
		break;
	/*
	 * second row address start with 0x40*/
	case 1:
		lcd_memory_address=a_col+SECOND_ROW_ADDRESS;
		break;
	/*
	 * third row address start with 0x10*/
	case 2:
		lcd_memory_address=a_col+THIRD_ROW_ADDRESS;
		break;
	/*
	* fourth row address start with 0x50*/
	case 3:
		lcd_memory_address=a_col+FOURTH_ROW_ADDRESS;
		break;
	}
